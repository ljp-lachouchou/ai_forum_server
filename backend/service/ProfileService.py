...
class ProfileService:
    ...
    def get_profile(self, user_id: UUID):
        """获取指定 ID 的用户信息"""
        res = self.client.table("profiles").select("*").eq("id", str(user_id)).single().execute()
        return res.data
    ...
    def update_profile(self, user_id: UUID, data: dict):
        ...
        res = self.client.table("profiles").update(data).eq("id", str(user_id)).execute()
        return res.data

    @AsyncWrapper.to_async(AsyncWrapper._upload_sem)
    def upsert_profile(self, profile_data: ProfileSchema):
        """创建或覆盖用户信息"""
        res = self.client.table("profiles").upsert(profile_data.model_dump(mode='json')).execute()
        return res.data

    @AsyncWrapper.to_async(AsyncWrapper._upload_sem)
    def insert_profile(self, profile_data: ProfileSchema):
        """创建或覆盖用户信息"""
        res = self.client.table("profiles").insert(profile_data.model_dump(mode='json')).execute()
        return res.data
