...
class LikeServiceTests(unittest.IsolatedAsyncioTestCase):
    async def test_add_like(self):
        sb = _StubSupabaseClient()
        service = LikeService(sb)
        user_id = uuid4()
        post_id = uuid4()

        await service.add_like(user_id, post_id)

        self.assertEqual(len(sb.insert_calls), 1)
        table, data = sb.insert_calls[0]
        self.assertEqual(table, "post_likes")
    ...
    async def test_remove_like(self):
        ...
        await service.remove_like(user_id, post_id)
    ...
    async def test_is_liked_true(self):
        ...
        result = await service.is_liked(user_id, post_id)
    ...
    async def test_toggle_like_adds_when_not_liked(self):
        sb = _StubSupabaseClient()
        sb.select_return = []
        service = LikeService(sb)
        user_id = uuid4()
        ...
        result = await service.toggle_like(user_id, post_id)

        self.assertTrue(result)
        self.assertEqual(len(sb.insert_calls), 1)
        self.assertEqual(len(sb.delete_calls), 0)

    async def test_toggle_like_removes_when_liked(self):
        sb = _StubSupabaseClient()
        sb.select_return = [{"id": "1"}]
        service = LikeService(sb)
        user_id = uuid4()
        ...
        self.assertEqual(len(sb.insert_calls), 0)
        self.assertEqual(len(sb.delete_calls), 1)

    async def test_list_user_likes(self):
        sb = _StubSupabaseClient()
        sb.select_return = [{"post_id": "1"}]
        service = LikeService(sb)
        user_id = uuid4()

        result = await service.list_user_likes(user_id)
